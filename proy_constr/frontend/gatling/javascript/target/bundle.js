var gatling = (() => {
  ...
  var require_checks = __commonJS({
    "node_modules/@gatling.io/core/target/checks/index.js"(exports) {
      ...
      exports.form = form;
      var jsonPath2 = (path) => (0, jsonOfTypeMultipleFind_1.wrapCheckBuilderJsonOfTypeMultipleFind)(typeof path === "function" ? jvm_types_1.CoreDsl.jsonPath((0, session_1.underlyingSessionTo)(path)) : jvm_types_1.CoreDsl.jsonPath(path));
      exports.jsonPath = jsonPath2;
  ...
  var require_groups = __commonJS({
    "node_modules/@gatling.io/core/target/structure/groups.js"(exports) {
      ...
      var session_1 = require_session();
      var on_1 = require_on();
      var groupImpl = (jvmGroups, wrap) => (group2) => (0, on_1.wrapOn)(typeof group2 === "function" ? jvmGroups.group((0, session_1.underlyingSessionTo)(group2)) : jvmGroups.group(group2), wrap);
  ...
  var basicSimulation_gatling_default = (0, import_core.simulation)((setUp) => {
    ...
    const registroUsuario = (0, import_core.scenario)("Registro Usuario").feed(crearFeederUsuarios(vu, "registro")).exec(
      (0, import_core.group)("Registro Usuario").on(
        (0, import_http.http)("Confirmar registro de usuario").post("/api/usuarios/registro").body(
        ...
        ).check((0, import_http.status)().is(201)).check((0, import_core.jsonPath)("$.usuario.id").exists())
      )
    );
    const inicioSesionUsuario = (0, import_core.scenario)("Inicio Sesion Usuario").feed(crearFeederUsuarios(vu, "registro")).exec(
      (0, import_core.group)("Inicio Sesion Usuario").on(
        (0, import_http.http)("Acceder al sistema").post("/api/usuarios/login").body(
          (0, import_core.StringBody)(
            (session) => JSON.stringify({
              email: session.get("email"),
              password: session.get("password")
    ...
    const creacionSolicitud = (0, import_core.scenario)("Creacion de solicitud").feed(crearFeederUsuarios(vu, "solicitud", 1e3)).exec(
      ...
      ).check((0, import_http.status)().is(201)).check((0, import_core.jsonPath)("$.usuario.id").saveAs("usuarioId"))
    ).exitHereIfFailed().exec(
      (0, import_http.http)("Iniciar sesion para solicitud").post("/api/usuarios/login").body(
        (0, import_core.StringBody)(
          (session) => JSON.stringify({
            email: session.get("email"),
            password: session.get("password")
      ...
      (0, import_core.group)("Creacion de solicitud").on(
